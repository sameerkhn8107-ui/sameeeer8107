...
# Firebase Admin SDK (optional - for delete-account and token verification)
try:
...
# Load .env from backend folder first, then from project root (parent) as fallback
load_dotenv(ROOT_DIR / '.env')
load_dotenv(ROOT_DIR.parent / '.env')

app = FastAPI()
origins = ["http://localhost:3000"]  # ये तुम्हारे frontend का address है
...
# ============== Firebase Admin (for delete-account & token verification) ==============
# Initialize only if credentials are provided and app not already initialized
_firebase_app = None
if FIREBASE_ADMIN_AVAILABLE:
    # Check if Firebase Admin is already initialized
    if not firebase_admin._apps:
        cred_path = os.environ.get("FIREBASE_CREDENTIALS_PATH") or os.environ.get("FIREBASE_SERVICE_ACCOUNT_PATH") or os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")
        if cred_path and Path(cred_path).exists():
            try:
                ...
                logger.info("✅ Delete-account endpoint is now enabled and ready")
            except Exception as e:
                logger.error(f"❌ Firebase Admin init failed: {e}. Delete-account endpoint will return 503.")
        else:
            logger.warning(f"⚠️ FIREBASE_CREDENTIALS_PATH not set or file missing at: {cred_path}")
            logger.warning("⚠️ Delete-account endpoint disabled until configured.")
    else:
        _firebase_app = firebase_admin.get_app()
        logger.info("✅ Firebase Admin SDK already initialized (reloaded)")
...
class ChatRequest(BaseModel):
    ...
    user_id: Optional[str] = None  # For daily limit enforcement
...
async def check_daily_limit(user_id: str) -> tuple[bool, int]:
    """
    Check if user has exceeded daily message limit.
    Returns (can_send, remaining_messages)
    """
    try:
        db = firestore.client()
        
        # Get current date in server timezone (UTC)
        from datetime import datetime, timezone
        now = datetime.now(timezone.utc)
        today_str = now.strftime("%Y-%m-%d")
        
        # Get or create user limit document
        ...
        if limit_doc.exists:
            ...
            if stored_date != today_str:
                ...
                logger.info(f"🔄 Reset daily limit for user {user_id[:8]}... (new day)")
                return True, DAILY_MESSAGE_LIMIT
            
            # Check if limit exceeded
            if message_count >= DAILY_MESSAGE_LIMIT:
                remaining = 0
                logger.warning(f"🚫 User {user_id[:8]}... exceeded daily limit ({message_count}/{DAILY_MESSAGE_LIMIT})")
                return False, remaining
            
            remaining = DAILY_MESSAGE_LIMIT - message_count
            logger.info(f"✅ User {user_id[:8]}... has {remaining} messages remaining")
            return True, remaining
        else:
            # First time user - create limit document
            limit_ref.set({
                "date": today_str,
                "count": 0,
                "last_reset": now
            })
            logger.info(f"📝 Created daily limit tracker for user {user_id[:8]}...")
            return True, DAILY_MESSAGE_LIMIT
...
async def increment_message_count(user_id: str):
    """
    Increment user's daily message count after successful message send.
    """
    try:
        db = firestore.client()
        ...
        if limit_doc.exists:
            ...
            if stored_date == today_str:
                ...
                logger.info(f"📊 User {user_id[:8]}... message count: {new_count}/{DAILY_MESSAGE_LIMIT}")
            else:
                # New day, reset
                limit_ref.set({
                    "date": today_str,
...
async def live_search(q: str):
    """
    Live search endpoint using Tavily API.
    ...
    async def generate():
        try:
            ...
            if request.user_id:
                ...
                if not can_send:
                    logger.warning(f"🚫 User {request.user_id[:8]}... blocked: daily limit reached")
                    yield f"data: {json.dumps({{'error': 'daily_limit', 'message': 'You have reached your daily limit of {DAILY_MESSAGE_LIMIT} messages. Please try again tomorrow.'}})}\n\n"
                    return
            ...
            if request.active_mode and detect_mode_deactivation(last_user_msg):
                ...
                if needs_live_search(last_user_msg) and TAVILY_API_KEY:
                    ...
                    logger.info(f"📚 Retrieved {len(sources)} sources for mode deactivation")
            ...
            # Send sources after AI response is complete (only for normal mode with live search)
    ...
    try:
        ...
        if request.user_id:
            ...
            if not can_send:
                ...
                return {
                    "error": "daily_limit",
                    "message": f"You have reached your daily limit of {DAILY_MESSAGE_LIMIT} messages. Please try again tomorrow.",
                    "success": False
                }
        ...
        if mode_action == "deactivate" or not request.active_mode:
            ...
            if needs_live_search(last_user_msg) and TAVILY_API_KEY:
                live_context, sources = await search_tavily(last_user_msg)
                logger.info(f"📚 Retrieved {len(sources)} sources for simple chat (else)")
            system_message = get_default_prompt(user_name, request.user_memory, live_context)
...
def _get_uid_from_token(authorization: Optional[str] = Header(None)) -> str:
    """
    Verify Firebase ID token from Authorization: Bearer <token> and return the user's UID.
    Raises HTTPException if token is missing, invalid, or Firebase Admin is not configured.
    """
    if not FIREBASE_ADMIN_AVAILABLE or _firebase_app is None:
        logger.error("❌ Delete account called but Firebase Admin SDK is not configured")
        raise HTTPException(
            status_code=503,
            detail="Delete account service is not configured. Firebase Admin SDK initialization failed."
        )
    if not authorization or not authorization.startswith("Bearer "):
        logger.warning("❌ Missing or invalid Authorization header in delete-account request")
        raise HTTPException(
            status_code=401, 
    ...
    if not id_token:
        logger.warning("❌ Empty Firebase ID token in delete-account request")
    ...
    try:
        ...
        logger.info(f"✅ Token verified successfully for UID: {uid[:8]}...")
        return uid
...
@api_router.delete("/delete-account")
async def delete_account(authorization: Optional[str] = Header(None)):
    """
    Permanently delete the authenticated user's account:
    - Verifies Firebase ID token (Authorization: Bearer <token>)
    - Deletes all Firestore data for that user (users, user_memory, conversations, messages)
    - Deletes the Firebase Auth user
    Returns JSON { "success": true } on success. Client should then log out and redirect to login.
    """
    logger.info("🗑️ Delete account request received")
    
    try:
        # Step 1: Verify token and get UID
        uid = _get_uid_from_token(authorization)
        logger.info(f"📋 Starting account deletion for UID: {uid[:8]}...")
        
        # Step 2: Initialize Firestore client
        db = firestore.client()
        logger.info("🔥 Firestore client initialized")

        # Step 3: Get all conversation IDs for this user
        logger.info(f"📂 Fetching conversations for user {uid[:8]}...")
        conv_refs = db.collection("conversations").where("userId", "==", uid).stream()
        conv_ids = [doc.id for doc in conv_refs]
        logger.info(f"📂 Found {len(conv_ids)} conversations to delete")

        # Step 4: Delete all messages in those conversations (batch to avoid query limits)
        total_messages_deleted = 0
        for cid in conv_ids:
            msg_refs = db.collection("messages").where("conversationId", "==", cid).stream()
            msg_count = 0
            for doc in msg_refs:
                doc.reference.delete()
                msg_count += 1
            total_messages_deleted += msg_count
            if msg_count > 0:
                logger.info(f"🗑️ Deleted {msg_count} messages from conversation {cid[:8]}...")
        logger.info(f"✅ Deleted total {total_messages_deleted} messages")

        # Step 5: Delete conversation documents
        for cid in conv_ids:
            db.collection("conversations").document(cid).delete()
        logger.info(f"✅ Deleted {len(conv_ids)} conversations")

        # Step 6: Delete user_memory document
        try:
            db.collection("user_memory").document(uid).delete()
            logger.info(f"✅ Deleted user_memory for {uid[:8]}")
        except Exception as e:
            logger.warning(f"⚠️ Error deleting user_memory (may not exist): {e}")

        # Step 7: Delete users document
        try:
            db.collection("users").document(uid).delete()
            logger.info(f"✅ Deleted users document for {uid[:8]}")
        except Exception as e:
            logger.warning(f"⚠️ Error deleting users document (may not exist): {e}")

        # Step 8: Delete Firebase Auth user (revokes tokens and removes identity)
        try:
            auth.delete_user(uid)
            logger.info(f"✅ Deleted Firebase Auth user for {uid[:8]}")
        except auth.UserNotFoundError:
            logger.warning(f"⚠️ Firebase Auth user {uid[:8]} not found (may already be deleted)")
        except Exception as e:
            logger.error(f"❌ Error deleting Firebase Auth user: {e}")
            raise HTTPException(
                status_code=500, 
                detail="Failed to delete authentication user. Please try again."
            )
        
        logger.info(f"🎉 Account deletion completed successfully for UID: {uid[:8]}")
        return {"success": True, "message": "Account deleted successfully."}
        
    except HTTPException:
        # Re-raise HTTP exceptions (401, 503) as-is
        raise
    except Exception as e:
        logger.exception(f"❌ Unexpected error during account deletion: {e}")
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to delete account due to internal error. Please try again later."
        )
...
app.add_middleware(
    CORSMiddleware,
    allow_credentials=True,
    allow_origins=["http://localhost:3000", "https://settings-delete-fail.preview.emergentagent.com"],
    allow_methods=["*"],
    allow_headers=["*"]
)
